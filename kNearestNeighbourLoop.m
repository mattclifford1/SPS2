function kNearestNeighbourLoop(feature1,feature2)
% close all
...
for kn = [1,3,5,10]
    ...
    for i = 1:length(pngFilesS)
        ...
        ind = knnsearch(train,testS(i,:),'k',kn);
        a = 0; b = 0; c = 0;
            for k = 1:kn
                if ind(k) > 0 && ind(k) < 11
                    a = a +1;
    ...
    for i = 1:length(pngFilesT)
        ...
        ind = knnsearch(train,testT(i,:),'k',kn);
        a = 0; b = 0; c = 0;
            for k = 1:kn
                if ind(k) > 0 && ind(k) < 11
                    a = a +1;
    ...
    for i = 1:length(pngFilesV)
        ...
        ind = knnsearch(train,testV(i,:),'k',kn);
        a = 0; b = 0; c = 0;
            for k = 1:kn
                if ind(k) > 0 && ind(k) < 11
                    a = a +1;
    ...
    dot = 20; do1 = 5;
    subplot(1,4,plt);plot(A(:,1),A(:,2),'.','color',[.8 1 1],'MarkerSize',dot);
    hold on
    subplot(1,4,plt);plot(B(:,1),B(:,2),'.','color',[1 .8 1],'MarkerSize',dot);
    subplot(1,4,plt);plot(C(:,1),C(:,2),'.','color',[1 1 .8],'MarkerSize',dot);
    subplot(1,4,plt);plot(feature1(1:10),feature2(1:10),'o','color',[0 0.5 0.5],'MarkerSize',do1)
    subplot(1,4,plt);plot(feature1(11:20),feature2(11:20),'x','color',[0.5 0 0.5],'MarkerSize',do1)
    subplot(1,4,plt);plot(feature1(21:30),feature2(21:30),'d','color',[0.5 0.5 0],'MarkerSize',do1)
    subplot(1,4,plt);plot(testS(:,1),testS(:,2),'k*','MarkerSize',2)
